...
• Parametric insurance: agricultural risk transfer products use drought indices, particularly SPI and SPEI to quantify drought risk and to define indemnity triggers (https://documents1.worldbank.org/curated/en/704171524632990898/pdf/Technical-Note.pdf). Parametric insurance covers have become increasingly available for agricultural assets including crops, livestock and forestory, with these products relying on regional drought indices. As an example, the Government of Indonensia (GoI) have developed parametric insurance products for the location of Central Java that utilise the SPI and SPEI index.
```

Further considerations

```
    •  Distribution parameters: ERA5‑Drought data are provided with SPI and SPEI indices that are ready for use without any need for preprocessing. The dataset could be made even more useful if the distribution parameters applied in calculating the SPI/SPEI cumulative distribution functions were included, specified for each calendar month and accumulation window. This would enable users to compute the indices from alternative datasets or to apply the parameters when generating forecast drought indices.

    •  CDS Format: ERA5-Drought data are served from the CDS in NetCDF format. When attempting to download quality‑flag data for multiple accumulation windows simultaneously, only the final accumulation window was returned. Until now, this behaviour was not documented, and it means that users must submit separate CDS requests for each accumulation window- hindering efficient filtering of quality-flagged data. In addition, the ensemble dataset contains duplicate timestamps for each ensemble member. Whilst individual ensemble members can still be accessed, it is considerably easier for users if a "member" dimension was added. This also avoids the complication of downloading and loading multiple accumulation windows into single xarray, where duplicated timestamps (accumulation window x number of members) can lead to alignment issues. 
